  ...
  function runProcess(pno_list) {
    ...
    setTimeout(() => {
      ...
      pno_list.forEach((pno) => {
        if (chopsticks[pno - 1] == 1 && chopsticks[pno % 5] == 1) {  // Condition to check which philosopher both the chopsticks
          ...
          pno_list = $.grep(pno_list, function (value) { // Removing the philosopher from list
            return value != pno;
          });
      ...
      // Function to take back the resources
      setTimeout(() => {
        allocated_phil.forEach((pno) => {
          ...
          // to enable the button after we finish our process of eating
